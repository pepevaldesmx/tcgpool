 ...
 *
 * A diferencia del buscador público, aquí NO se filtra por stock: la tienda
 * captura justamente lo que nadie tiene publicado, así que esconder lo agotado
 * escondería casi todo lo que quiere capturar.
 *
...
export async function GET(request: Request) {
  ...
  const cards = (await searchCards(q, { limit: 6, onlyInStock: false })).slice(0, 6);
